...
imageItems.forEach((item, index) => {
    ...
    img.onload = () => {
        ...
        const itemHeight = imgHeight + title.offsetHeight + 30; // 图片高度 + 标题高度 + 间距
...
function isOverlapping(x, y, width, height) {
    return positions.some(pos => {
        return !(
            x + width + spacing < pos.x || // 在已放置图片的左侧
            x > pos.x + pos.width + spacing || // 在已放置图片的右侧
            y + height + spacing < pos.y || // 在已放置图片的上方
            y > pos.y + pos.height + spacing // 在已放置图片的下方
        );
    });
}
});
